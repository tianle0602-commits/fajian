...
import { analyzeLaborDoc } from "@/lib/review-engine";
...
export default function LaborPage() {
  ...
  const handleReview = async () => {
    ...
    try {
      ...
      const data = analyzeLaborDoc(file.name, content);
      setResult(data);
    } catch {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8 sm:py-12">
      <div className="mb-8 sm:mb-10 text-center">
        <h1 className="mb-3 text-2xl font-bold text-primary sm:text-3xl md:text-4xl">劳动合规</h1>
        <p className="text-sm text-gray-500 sm:text-base">
          上传企业用工制度文件，AI 全面检查合规性，覆盖劳动合同、社保、工时等关键领域
        </p>
      </div>
